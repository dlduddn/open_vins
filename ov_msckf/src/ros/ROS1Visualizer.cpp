...
bool ROS1Visualizer::request_matlab_constraint_update(const MatlabConstraintSnapshot &snapshot, MatlabConstraintUpdate &update) {
  ...
  PRINT_INFO("[MATLAB] constraint request #%llu: t_cam=%.6f t_imu=%.6f p=[%.6f %.6f %.6f] q=[%.6f %.6f %.6f %.6f]\n",
             (unsigned long long)request_index, snapshot.timestamp_cam, snapshot.timestamp_imu, snapshot.position(0), snapshot.position(1),
             snapshot.position(2), snapshot.quaternion(0), snapshot.quaternion(1), snapshot.quaternion(2), snapshot.quaternion(3));
  ...
  PRINT_INFO("[MATLAB] constraint service call begin #%llu\n", (unsigned long long)request_index);
  const bool call_success = matlab_snapshot_client.call(snapshot_srv);
  PRINT_INFO("[MATLAB] constraint service result #%llu: success=%d accepted=%d status=\"%s\"\n", (unsigned long long)request_index,
             (int)call_success, call_success ? (int)snapshot_srv.response.accepted : 0,
